...
import { PinMetadataDto, pinMetadataSchema } from './dto/pin-metadata.dto';
import { OptimizeImageDto, optimizeImageSchema } from './dto/optimize-image.dto';
import { VerifyHashDto, verifyHashSchema } from './dto/verify-hash.dto';
...
export class StorageController {
  ...
  async pinProjectMetadata(@Body(new ZodValidationPipe(pinMetadataSchema)) dto: PinMetadataDto): Promise<string> {
  ...
  async optimizeAndUploadBanner(@Body(new ZodValidationPipe(optimizeImageSchema)) dto: OptimizeImageDto): Promise<string> {
  ...
  async verifyIPFSHash(@Body(new ZodValidationPipe(verifyHashSchema)) dto: VerifyHashDto): Promise<boolean> {
  ...
  async getPresignedUrl(@Body(new ZodValidationPipe(presignUrlSchema)) dto: PresignUrlDto): Promise<{ url: string }> {
